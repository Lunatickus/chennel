...
from .transliteration import transliteration_rus_eng_image as translite


def create_path(instance, filename):
    """Определение пути сохранения изображеня"""
    ...
    def full_clean(self, *args, **kwargs):
        ...
        if not self.check_size():
            raise DjangoValidationError(
                'Размер изображения быть не менее {}x{} пикселей'.format(self.min_height, self.min_width)
            )
